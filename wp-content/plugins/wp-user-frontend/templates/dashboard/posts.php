...
if ( isset( $_GET['msg'] ) && $_GET['msg'] == 'deleted' ) {
    echo '<div class="success">' . __( 'Trabalho excluído', 'wp-user-frontend' ) . '</div>';
}
...
<?php if ( wpuf_get_option( 'show_post_count', 'wpuf_dashboard', 'on' ) == 'on' ) { ?>
    <div class="post_count"><?php printf( __( 'Você tem <span>%d</span> %s<br><br>', 'wp-user-frontend' ), $dashboard_query->found_posts, $post_type_obj->label ); ?></div>
<?php } ?>
    ...
    <table class="items-table <?php echo $post_type; ?>" cellpadding="0" cellspacing="0">
        <thead>
            <tr class="items-list-header">
                <?php
                if ( 'on' == $featured_img ) {
                    echo '<th>' . __( 'Imagem', 'wp-user-frontend' ) . '</th>';
                }
                ?>
                <th><?php _e( 'Título', 'wp-user-frontend' ); ?></th>
                ...
                <th><?php _e( 'Opções', 'wp-user-frontend' ); ?></th>
            </tr>
        </thead>
        <tbody>
            <?php
                ...
                <tr>
                    ...
                    <td>
                        ...
                        if ( wpuf_get_option( 'enable_post_edit', 'wpuf_dashboard', 'yes' ) == 'yes' ) {
                            ...
                            if ( $show_edit ) {
                                ?>
                                <a class="wpuf-posts-options wpuf-posts-edit" href="<?php echo wp_nonce_url( $url, 'wpuf_edit' ); ?>"><?php _e( 'Editar', 'wp-user-frontend' ); ?></a><br><br>
                                <?php
                            }
                        }
                        ?>
                        ...
                        if ( wpuf_get_option( 'enable_post_del', 'wpuf_dashboard', 'yes' ) == 'yes' ) {
                            ...
                            $message = __( 'Tem certeza que deseja deletar?', 'wp-user-frontend' );
                            ?>
                            <a class="wpuf-posts-options wpuf-posts-delete" style="color: red;" href="<?php echo wp_nonce_url( $del_url, 'wpuf_del' ) ?>" onclick="return confirm('<?php echo $message ?>');"><?php _e( 'Apagar', 'wp-user-frontend' ); ?></a>
                        <?php } ?>
                    </td>
                </tr>
                <?php
