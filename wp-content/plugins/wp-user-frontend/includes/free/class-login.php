...
class WPUF_Simple_Login {
    ...
    function get_action_links( $args = array() ) {
        ...
        if ( $args['register'] ) {
            $links[] = sprintf( '<a href="%s">%s</a>', $this->get_action_url( 'register' ), __( 'Registrar', 'wp-user-frontend' ) );
        }

        if ( $args['lostpassword'] ) {
            $links[] = sprintf( '<a href="%s">%s</a>', $this->get_action_url( 'lostpassword' ), __( 'Esqueceu a senha?', 'wp-user-frontend' ) );
        }
        ...
        if ( is_user_logged_in() ) {
            ...
            switch ($action) {
                case 'lostpassword':
                    ...
                    $this->messages[] = __( 'Por favor, entre com seu endereço de email ou usuário. Você receberá um link para criar uma nova senha via email.', 'wp-user-frontend' );
                ...
                case 'resetpass':
                    ...
                    if ( isset( $_GET['reset'] ) && $_GET['reset'] == 'true' ) {
                        ...
                        printf( '<div class="wpuf-message">' . __( 'Sua senha foi renovada. %s', 'wp-user-frontend' ) . '</div>', sprintf( '<a href="%s">%s</a>', $this->get_action_url( 'login' ), __( 'Log In', 'wp-user-frontend' ) ) );
                        return;
                    } else {

                        $this->messages[] = __( 'Entre com sua nova senha abaixo..', 'wp-user-frontend' );
                ...
                default:
                    ...
                    if ( isset( $_GET['checkemail'] ) && $_GET['checkemail'] == 'confirm' ) {
                        $this->messages[] = __( 'Cheque seu email para encontrar o link de confirmação.', 'wp-user-frontend' );
                    }

                    if ( isset( $_GET['loggedout'] ) && $_GET['loggedout'] == 'true' ) {
                        $this->messages[] = __( 'Você está deslogado.', 'wp-user-frontend' );
                    }
    ...
    function process_login() {
        if ( !empty( $_POST['wpuf_login'] ) && !empty( $_POST['_wpnonce'] ) ) {
            ...
            if ( empty( $_POST['log'] ) ) {
                $this->login_errors[] = __( 'Usuário é obrigatório.', 'wp-user-frontend' );
                return;
            }

            if ( empty( $_POST['pwd'] ) ) {
                $this->login_errors[] = __( 'Senha é obrigatória.', 'wp-user-frontend' );
                return;
            }
