    ...
    function process_registration() {
        if ( !empty( $_POST['wpuf_registration'] ) && !empty( $_POST['_wpnonce'] ) ) {
            ...
            if ( empty( $_POST['reg_fname'] ) ) {
                $this->registration_errors[] = '<strong>' . __( 'Error', 'wp-user-frontend' ) . ':</strong> ' . __( 'Primeiro nome é obrigatório.', 'wp-user-frontend' );
                return;
            }

            if ( empty( $_POST['reg_lname'] ) ) {
                $this->registration_errors[] = '<strong>' . __( 'Error', 'wp-user-frontend' ) . ':</strong> ' . __( 'Último nome é obrigatório.', 'wp-user-frontend' );
                return;
            }

            if ( empty( $_POST['reg_email'] ) ) {
                $this->registration_errors[] = '<strong>' . __( 'Error', 'wp-user-frontend' ) . ':</strong> ' . __( 'Email é obrigatório.', 'wp-user-frontend' );
                return;
            }

            if ( empty( $_POST['log'] ) ) {
                $this->registration_errors[] = '<strong>' . __( 'Error', 'wp-user-frontend' ) . ':</strong> ' . __( 'Nome do usuário é obrigatório.', 'wp-user-frontend' );
                return;
            }

            if ( empty( $_POST['pwd1'] ) ) {
                $this->registration_errors[] = '<strong>' . __( 'Error', 'wp-user-frontend' ) . ':</strong> ' . __( 'Senha é obrigatória.', 'wp-user-frontend' );
                return;
            }

            if ( empty( $_POST['pwd2'] ) ) {
                $this->registration_errors[] = '<strong>' . __( 'Error', 'wp-user-frontend' ) . ':</strong> ' . __( 'Confirmar a senha é obrigatório.', 'wp-user-frontend' );
                return;
            }

            if ( $_POST['pwd1'] != $_POST['pwd2'] ) {
                $this->registration_errors[] = '<strong>' . __( 'Error', 'wp-user-frontend' ) . ':</strong> ' . __( 'As senhas não são a mesma.', 'wp-user-frontend' );
                return;
            }

            if ( get_user_by( 'login', $_POST['log'] ) === $_POST['log'] ) {
                $this->registration_errors[] = '<strong>' . __( 'Error', 'wp-user-frontend' ) . ':</strong> ' . __( 'Já existe alguém com este usuário.', 'wp-user-frontend' );
                return;
            }
            ...
            if ( is_email( $_POST['log'] ) && apply_filters( 'wpuf_get_username_from_email', true ) ) {
                ...
                if ( isset( $user->user_login ) ) {
                    ...
                    $this->registration_errors[] = '<strong>' . __( 'Error', 'wp-user-frontend' ) . ':</strong> ' . __( 'Não foi possível encontrar um usuário com este endereço de email.', 'wp-user-frontend' );
                    return;
                }
            } else {
            ...
            if ( is_wp_error( $user ) ) {
                ...
                if ( !empty( $_POST['redirect_to'] ) ) {
                    $redirect = esc_url( home_url( '/logado' ) );
                } else {
                    $redirect = esc_url( home_url( '/logado' ) );
                }
